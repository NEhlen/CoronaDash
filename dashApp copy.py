...
app.layout = dbc.Container([
    ...
    dbc.Card([
        ...
        dbc.CardBody([
            dbc.Row(
                dcc.Graph(id='gr-death-confirmed',figure=death_confirmed_plot(dFrames['deaths'][('Total', 'tot')].drop(['Canada', 'China', 'Australia']).iloc[:-1], dFrames['confirmed'][('Total', 'tot')].drop(['Canada', 'China', 'Australia']).iloc[:-1]))
            ),
        ]
        )
    ])
...
def update_world(tabs, date, switch):
    ...
    if date > len(df['Data'].columns.values):
        datsize = df['Total', 'tot'].iloc[:-1]
        dateDisplay = 'Total'
        datnormal = np.max(datsize)
    else:
        datsize = df['Data'][df['Data'].columns.values[date]].iloc[:-1]
    ...
    if 0 not in switch:
        datnormal = np.amax(df['Data'].fillna(0).iloc[:-1].to_numpy())
        print("Normalize World with: ", datnormal)


    data = [
        go.Scattergeo(
            lat=df[('Coords', 'Lat')].iloc[:-1],
            lon=df[('Coords', 'Lon')].iloc[:-1],
            marker = dict(
            ...
            text= [str(datsize.index.values[i]) + "\n" + str(datsize[i]) for i in range(len(datsize))]
        )
    ]
    #layout_kwargs = layout_dict.get(tabs,None)
    layout = go.Layout(
